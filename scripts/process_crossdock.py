...
def process_ligand_and_pocket(pdbfile, sdffile,
                              atom_dict, amino_acid_dict, dist_cutoff, ca_only, backbone=False):
    ...
    filtered_atoms = [(a.GetSymbol(), idx) for idx, a in enumerate(ligand.GetAtoms())
                      if (a.GetSymbol().capitalize() in atom_dict or a.GetSymbol() != 'H')]
    
    # Tách ra danh sách ký hiệu nguyên tử
    lig_atoms = [a[0] for a in filtered_atoms]
    
    # Chỉ lấy tọa độ của các index đã lọc
    lig_coords = np.array([list(ligand.GetConformer(0).GetAtomPosition(a[1]))
                           for a in filtered_atoms])
    
    sdf_idx_to_new_idx = {old_idx: new_idx for new_idx, (sym, old_idx) in enumerate(filtered_atoms)}
    bonds = []
    if ligand is not None:
        for bond in ligand.GetBonds():
            ...
            # Chỉ giữ bond nếu cả 2 đầu đều là atom được giữ lại
            if idx1 in sdf_idx_to_new_idx and idx2 in sdf_idx_to_new_idx:
                new_idx1 = sdf_idx_to_new_idx[idx1]
                new_idx2 = sdf_idx_to_new_idx[idx2]
                
                # Thêm 2 chiều (undirected) để tương thích PyG
                bonds.append([new_idx1, new_idx2])
                bonds.append([new_idx2, new_idx1])
    
    if len(bonds) > 0:
        # Transpose để có shape [2, num_bonds]
        lig_bonds = np.array(bonds, dtype=np.int64).T
    else:
        lig_bonds = np.empty((2, 0), dtype=np.int64)
    ...
    if ca_only:
        ...
        try:
            pocket_one_hot = []
            full_coords = []
            target_atoms = ['N', 'CA', 'C', 'O'] # Các nguyên tử backbone cần lấy
            valid_pocket_ids = []

            for res in pocket_residues:
                # Kiểm tra các nguyên tử backbone có trong residue không
                if all(atom_name in res for atom_name in target_atoms):
                    for atom_name in target_atoms:
                        if atom_name in res:
                            atom = res[atom_name]
                            full_coords.append(atom.coord)
                            
                            # QUAN TRỌNG: Dùng amino_acid_dict để mã hóa loại Acid Amin
                            # (Cả 4 nguyên tử đều mang thông tin loại AA của residue đó)
                            aa_code = three_to_one(res.get_resname())
                            pocket_one_hot.append(np.eye(1, len(amino_acid_dict),
                                                        amino_acid_dict[aa_code]).squeeze())
                valid_pocket_ids.append(f'{res.parent.id}:{res.id[1]}')
                
            if len(full_coords) == 0:
                 raise ValueError("No valid backbone residues found.")
            pocket_one_hot = np.stack(pocket_one_hot)
            full_coords = np.stack(full_coords)
            pocket_ids = valid_pocket_ids
...
def saveall(filename, pdb_and_mol_ids, lig_coords, receptors, lig_one_hot,lig_bonds, lig_mask,
    ...
    for split in data_split.keys():
        ...
        for pocket_fn, ligand_fn in pbar:
            ...
            try:
                ...
                pdbfile, sdffile,
                atom_dict=atom_dict, amino_acid_dict=amino_acid_dict, dist_cutoff=args.dist_cutoff,
                ca_only=args.ca_only, backbone=args.backbone)
            ...
            local_bonds = ligand_data['lig_bonds'] # [2, E]
            #num_atoms_in_mol = len(ligand_data['lig_coords'])

            lig_bonds_list.append(local_bonds)
            
            new_rec_name = Path(pdbfile).stem.replace('_', '-')
            receptor_filename = f"{new_rec_name}.pdb"
            receptors.append(receptor_filename) 

            if split in {'val', 'test'}:
                pdb_file_out = Path(pdb_sdf_dir, receptor_filename)
                if not pdb_file_out.exists():
        ...
        lig_bonds = np.empty(len(lig_bonds_list), dtype=object)
        lig_bonds[:] = lig_bonds_list

        saveall(processed_dir / f'{split}.npz', pdb_and_mol_ids, lig_coords, receptors,
                lig_one_hot, lig_bonds, lig_mask, pocket_coords,
