...
class ProcessedLigandPocketDataset(Dataset):
    def __init__(self, npz_path, center=True, transform=None):
        ...
        for (k, v) in data.items():
            ...
            if k == 'lig_bonds':
                # Vì lig_bonds lưu dạng object array (mỗi phần tử là 1 mảng bond riêng), 
                # ta không cần np.split mà chỉ cần convert sang Tensor.
                # Kiểm tra nếu mảng rỗng thì tạo tensor rỗng.
                self.data[k] = []
                for b in v:
                    if b is None or len(b) == 0 or b.size == 0:
                        self.data[k].append(torch.empty((2, 0), dtype=torch.long))
                    else:
                        # Đảm bảo kiểu dữ liệu là Long (int64) cho index
                        self.data[k].append(torch.from_numpy(b).long())
                continue
    ...
    def collate_fn(batch):
        ...
        for prop in batch[0].keys():
            ...
            if prop == 'names' or prop == 'receptors':
                ...
                # Logic: Cần cộng dồn số lượng nguyên tử (offset) để index không bị trùng
                bonds_list = []
                offset = 0
                for x in batch:
                    b = x[prop] # [2, E]
                    num_atoms = x['lig_coords'].shape[0]
                    
                    if b.numel() > 0:
                        # Cộng offset vào index
                        bonds_list.append(b + offset)
                    else:
                        bonds_list.append(b)
                    
                    # Cập nhật offset cho mẫu tiếp theo
                    offset += num_atoms
                
                # Nối tất cả lại theo chiều ngang (dim=1) -> [2, Total_Edges]
                if len(bonds_list) > 0:
                    out[prop] = torch.cat(bonds_list, dim=1)
                else:
                    out[prop] = torch.empty((2, 0), dtype=torch.long)
