...
def plot_molecule(ax, positions, atom_type, alpha, spheres_3d, hex_bg_color,
    ...
    # =====================================================
    # CRITICAL FIX: Safe indexing for virtual node artifacts
    # =====================================================
    # Clamp atom_type to valid range to prevent IndexError
    # This handles cases where virtual nodes (index 10) leak through
    max_valid_idx = len(colors_dic) - 1
    atom_type_safe = np.clip(atom_type, 0, max_valid_idx)

    # Filter out invalid atoms (those that were clamped)
    valid_mask = atom_type <= max_valid_idx
    if not valid_mask.all():
        print(f"Warning: Found {(~valid_mask).sum()} atoms with invalid type indices "
              f"(max valid: {max_valid_idx}). These may be virtual node artifacts.")

    areas = area_dic[atom_type_safe]
    radii = radius_dic[atom_type_safe]
    colors = colors_dic[atom_type_safe]
    ...
    for i in range(len(x)):
        for j in range(i + 1, len(x)):
            ...
            # Use clamped atom_type for safe indexing
            atom1, atom2 = dataset_info['atom_decoder'][atom_type_safe[i]], \
                           dataset_info['atom_decoder'][atom_type_safe[j]]
            s = (atom_type_safe[i], atom_type_safe[j])
